...
$otaService = Read-Ui "src/OtaService.cpp"
$webService = Read-Ui "src/WebService.cpp"
...
Assert-Ui (($html -match 'ota-password-change') -and ($html -match 'otaCurrentPassword') -and ($html -match 'otaNewPasswordConfirm')) "OTA password change controls explain persistent credentials"
...
Assert-Ui ($js -match 'LIVE_CLOCK_INTERVAL_MS\s*=\s*1000') "live clock refresh interval is one second"
Assert-Ui ($js -match 'setInterval\(refreshStatus,\s*POLL_INTERVAL_MS\)') "status polling remains separate from clock refresh"
Assert-Ui ($js -match 'setInterval\(tickLiveClock,\s*LIVE_CLOCK_INTERVAL_MS\)') "live clock refresh is scheduled independently"
...
Assert-Ui (($js -match 'post\("/api/ota/password"') -and ($js -match 'changeOtaPassword')) "OTA password changes use a dedicated request"
...
Assert-Ui (($otaService -match 'bool OtaService::changePassword') -and ($webService -match 'handleOtaPasswordChange')) "firmware verifies the current password before changing OTA credentials"
